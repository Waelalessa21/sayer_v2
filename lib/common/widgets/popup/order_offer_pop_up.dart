...
class OrderOffersPopup extends StatefulWidget {
  const OrderOffersPopup({super.key, required this.subtitle});
  final String subtitle;

  @override
  State<OrderOffersPopup> createState() => _OrderOffersPopupState();
}

class _OrderOffersPopupState extends State<OrderOffersPopup> {
  bool isTyping = false;
  ...
  void _onSend() {
    ...
    if (showNameFields) {
      ...
      if (!textController.text.contains(" - ${widget.subtitle}")) {
        textController.text =
            "${textController.text.trim()} - ${widget.subtitle}";
      }
      cubit.suggestionCars(widget.subtitle);
      setState(() {
              ...
              Center(
                child: AnimatedItem(
                  ...
                  child: TextAndIcon(
                    ...
                    label: "وش تامر عليه",
                    description:
                        "بس علمني السيارة اللي تبيها، ${widget.subtitle}",
                    onPressed: () {},
                  ),
                ),
              ),
